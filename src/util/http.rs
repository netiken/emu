...
impl<R> HttpConnector<R> {
    ...
    /// Sets the TOS (Type of Service) value to be set on every packet sent on this socket.
    pub fn set_tos(&mut self, tos: Option<u32>) -> &mut Self {
        self.config_mut().tos = tos;
        self
    }
...
fn connect(
    ...
    if let Some(tos) = config.tos {
        socket
            .set_tos(tos)
            .map_err(ConnectError::m("tcp set tos error"))?;
    }
